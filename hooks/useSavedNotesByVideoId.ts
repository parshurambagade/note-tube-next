...
export const useSavedNotesByVideoId = (videoId: string) => {
  ...
  const fetchSavedNotes = async (videoId: string) => {
    try {
      ...
      const notes = await NotesService.getNotesByVideoId(videoId, user?.id as string);
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch saved note');
      setSavedNotes(null);
    } finally {
      setLoading(false);
    }
